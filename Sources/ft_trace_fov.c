...
/*   Updated: 2018/06/14 03:27:06 by allallem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_redirect(t_wolf3d *p)
{
	...
	else if (p->fov.xdiff > 0 && p->fov.ydiff < 0 && p->fov.xdiff >=
	ft_abs(p->fov.ydiff))
		ft_trace_bottom_right_left(p, 1);
	else if (p->fov.xdiff < 0 && p->fov.ydiff >= 0 && ft_abs(p->fov.xdiff)
	>= p->fov.ydiff)
...
void		ft_trace_fov(t_wolf3d *p)
{
	...
	p->fov.x2 = p->fov.x1 + ((p->v1.x + p->plane.x) * p->mini.xmap);
	p->fov.y2 = p->fov.y1 + ((p->v1.y + p->plane.y) * p->mini.xmap);
