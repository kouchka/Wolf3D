...
/*   Updated: 2018/06/08 17:23:16 by allallem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_trace_baton(t_wolf3d *p)
{
	...
	while (j < p->fov.ydiff)
	{
		ft_put_pixel(p, (p->fov.x1 * 4) + (p->image.size_line * p->fov.y1), 0x00FF00);
		p->fov.y1++;
		j++;
	}
}
...
void		ft_trace_up_left_right(t_wolf3d *p, int i)
{
	...
	while (j < p->fov.xdiff)
	{
		...
		if (i == 0)
			ft_put_pixel(p, ((p->fov.x1 - j) * 4) + (p->image.size_line * p->fov.y1),
					0x00FF00);
		else if (i == 1)
			ft_put_pixel(p, ((p->fov.x1 + j) * 4) + (p->image.size_line * p->fov.y1),
					0x00FF00);
...
void		ft_trace_up_right_left(t_wolf3d *p, int i)
{
	...
	while (j < p->fov.xdiff || j < p->fov.ydiff)
	{
		...
		ft_put_pixel(p, (((p->fov.x1 * 4) + ((p->image.size_line) * p->fov.y1))),
				0x00FF00);
		j++;
		p->fov.y1--;
		yjump += p->fov.temp;
